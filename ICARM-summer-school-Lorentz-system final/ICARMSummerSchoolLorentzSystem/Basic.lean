...
open Set
----Swati+Pankaj theorems and results to be used later in the main theorem
...
----Swati+Pankaj calculations on compactness of D, F and related properties
/-- If `x ^ 2 ≤ K` then `|x| ≤ max K 1`: either `|x| ≤ 1`, or `|x| ≤ |x| ^ 2 = x ^ 2 ≤ K`.
This gives a bound with no square roots. -/
...
lemma D_quadratic_le (p : LorenzParams) {x y z : ℝ} (hu : (x, y, z) ∈ D p) :
  ...
  simp only [D, mem_setOf_eq, LyapunovTilde] at hu
  nlinarith [p.σ_pos]
...
lemma F_isCompact (p : LorenzParams)
  ...
  rintro ⟨x, y, z⟩ hu
  simp only [F, Set.mem_setOf] at hu
...
---- These are the two different cases.
...
----This is the main theorem above, which we will prove by proving two seperate cases.
